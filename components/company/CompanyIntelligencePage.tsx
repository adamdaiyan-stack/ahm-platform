// components/company/CompanyIntelligencePage.tsx
//
// Generic Company Intelligence Page — the reusable framework.
//
// ARCHITECTURE
//   This component is the PLATFORM-LEVEL page shell.
//   It assembles all reusable company intelligence components
//   using a CompanyIntelligenceConfig provided by the caller.
//
//   Pattern:
//     1. Caller (page router) builds a CompanyIntelligenceConfig
//        from the company's static intelligence file.
//     2. This component renders the standard layout with that config.
//     3. No company-specific logic or hardcoded content lives here.
//
//   Future DB migration path:
//     Replace static config with:
//       const config = await getCompanyIntelligence(sym);
...
// All company-specific intelligence content is passed through this config.
// The page layout and component wiring is universal — only the config changes
// per company. This is the contract between the router and the framework page.

export type CompanyIntelligenceConfig = {
  // Visual identity
  accentColor:       string;   // Hex — used for accent stripe, sector badge, chart
  exchangeLabel?:    string;   // e.g. "Commercial Bank", "Oil & Gas Exploration" (defaults to company.sector)

  // Investment Thesis
  thesisSummary:     string;
  thesisThemes:      ThesisTheme[];
  ...
  valuationHistoricalRange?: string;
  valuationPeerContext?:   string;

  // Dividend Analysis
  dividendCommentary:       string;
  dividendYieldPositioning: string;
  dividendConsistencyNote:  string;

  // Peers section label override (default: "<sector> Peers")
  peersLabel?: string;
};
...
type CompanyIntelligencePageProps = {
...
export default function CompanyIntelligencePage({
  ...
  const exchangeLabel = config.exchangeLabel ?? company.sector;
  const peersLabel    = config.peersLabel    ?? (company.sector + " Peers");
  const waText        = encodeURIComponent(
    "I was looking at " + company.symbol + " on AHM Platform and want to get daily PSX updates"
  );
  ...
  return (
    <main className="min-h-screen bg-base text-tx-primary">
        ...
        <div className="px-6 md:px-8 pt-8 pb-6">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            ...
            <div>
              ...
              <p className="text-xs text-tx-disabled font-mono">PSX: KSE-100 · {exchangeLabel}</p>
            </div>
      ...
      <div className="px-6 md:px-8 py-8 max-w-6xl mx-auto space-y-10">
        ...
        {/* 1 — Investment Thesis */}
        <InvestmentThesis
          summary={config.thesisSummary}
          themes={config.thesisThemes}
          accentColor={accentColor}
        />

        {/* 2 — Key Drivers */}
        <CompanyDrivers drivers={config.drivers} />

        {/* 3 — Risks & Catalysts */}
        <RiskCatalystPanel risks={config.risks} catalysts={config.catalysts} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <DividendAnalysis
            dividends={dividends}
            commentary={config.dividendCommentary}
            yieldPositioning={config.dividendYieldPositioning}
            consistencyNote={config.dividendConsistencyNote}
          />
        ...
        {/* 6 — Valuation Summary */}
        <ValuationSummary
          points={config.valuationPoints}
          summary={config.valuationSummary}
          historicalRange={config.valuationHistoricalRange}
          peerContext={config.valuationPeerContext}
        />

        {/* 7 — Related Research + AI Placeholder */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <RelatedResearch reports={reports} symbol={company.symbol} />
          <AIInsightPlaceholder symbol={company.symbol} companyName={company.company_name} />
        </div>
        ...
        <section>
          ...
          <div
            className="bg-surface border border-border-theme rounded-xl flex flex-col items-center justify-center p-8"
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-tx-secondary text-xs mt-0.5">
                {"Start trading " + company.symbol + " and other KSE-100 stocks. Ready in 48 hours."}
              </p>
            </div>
